# -*- coding: utf-8 -*-
"""
Created on Wed Jul 29 09:22:12 2020
...
# Column 2 = input (u)
# Column 3 = output (yp)
data = np.loadtxt('data25uM3.txt',delimiter=',')
# u0 = data[0,1]
# yp0 = data[0,1]
# t = data[:,0].T - data[0,0]
t = data[:,0]
...
def fopdt(y,t,x,Km1,Km2,Km3,Km4,taum):
  ...
  #  try:
    if t <= 2:
                Km1 = x[0]
                dydt = Km1*y*(1-(y/taum))-Km4*y
    else: 
        if t <= 3:
                Km2 = x[1]
                dydt = Km2*y*(1-(y/taum))-Km4*y
        else:
                Km3 = x[2]
                dydt = Km3*y*(1-(y/taum))-Km4*y
  #  except:
...
plt.xlabel('Day')
plt.title('Best Fit Model - Logistic')
plt.legend(loc='best')
# plt.subplot(2,1,2)
